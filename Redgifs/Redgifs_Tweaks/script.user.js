...
JSON.parse = new Proxy(JSON.parse, {
	apply(target, thisArg, argList) {
		...
		if (jsonParsed?.gifs?.length) {
			console.log("JSON", jsonParsed);

			jsonParsed.gifs = jsonParsed.gifs.filter(
				(gif) => !gif.hasOwnProperty("cta"),
			);
...
const mutationsHandler = (mutations) => {
	for (const mutation of mutations) {
		for (const node of mutation.addedNodes) {
			...
			if (
				node?.classList?.contains("GifPreview-SideBarWrap") &&
				node.innerHTML !== ""
			) {
...
GM_addStyle(
	'.topNav-wrap > :not([class]) { visibility: hidden !important; } .SideBar-Item:has(> [class*="liveAdButton"]), [rel*="sponsored"] { display: none !important; } .download-progress-bar::before { position: absolute; height: 100%; background-color: #fff; content: "" ; width: var(--progress-bar, 0%); transition: .3s ease; }',
);
